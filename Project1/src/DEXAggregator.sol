...
contract DEXAggregator is Ownable, Pausable, ReentrancyGuard {
	...
	constructor(
		...
		supportedV3Fees[500] = true;
		supportedV3Fees[3000] = true;
		supportedV3Fees[10000] = true;
	...
	mapping(uint24 => bool) public supportedV3Fees;
		...
		returns (uint256 amountOut)
	{
		...
		for (uint24 fee = 500; fee <= 10000; fee *= 10) {
			if (supportedV3Fees[fee]) {
	...
	function setSupportedV3Fee(uint24 fee, bool enabled) external onlyOwner {
		supportedV3Fees[fee] = enabled;
	}
